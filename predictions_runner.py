...
def make_preds(data, model: ClipCaptionModel, out_path, tokenizer, dataset_mode, args=None):
    ...
    if dataset_mode == 0 or dataset_mode == 7 or dataset_mode == 8:
        ...
        images_root = './data/flickr30k_images'
...
def load_data(dataset_mode):
    if dataset_mode == 0:
        ...
        #        'r') as f:
        with open(
                f'./subset_jsons/train_subset_data1_100th.json',
                'r') as f:
            data = json.load(f)
    elif dataset_mode == 2:
        with open(
...
def main():
    ...
    model.load_state_dict(torch.load(args.checkpoint, map_location=CUDA(0)), strict=False)  # FIXME

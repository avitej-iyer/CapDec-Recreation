...
# dave conf
# ~~~~~~~~~
add_text_embedding = True    #originally set to true
        ...
        if not add_text_embedding:
            if images_path != 'NoImgs':
                if data_mode == 0:
                    ...
                    filename = images_path + str(img_id) + ".jpg"
...
def run_main():
    ...
    if args.dataset_mode == 0:
        ...
        out_path = f"./data/flicker30_{clip_model_name}_val.pkl"
        if add_text_embedding:
            out_path = f"./data/flicker30_{clip_model_name}_val_with_text_embeddings_not_norm.pkl"
            print(f'Text embeddings will be added to the dataset')
        annotations_path = f"./subset_jsons/val_1_10th.json"
        images_path = f"./data/flickr30k_images"
